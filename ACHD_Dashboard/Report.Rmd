...
pt.summary <- data.frame(
                        ...
                        hr.patient = area.achd %>% filter(as.numeric(shortest_time, "hours") <= 1) %>%
                            ...
                            mutate(ltf.percent = round( ltf.patient / no.patient, digits = 2 ), #LTF patients as a percentage of all patients
                                   hr.percent = round( hr.patient / no.patient, digits = 2 )) %>% #<hr patients as a perc of all patients
...
pt.summary %>%
  kbl(align=rep('r', 3), # right align the data
...
area.summary <- data.frame(
  ...
  avg.drive = avg.drive <- area.achd %>% 
                ...
                mutate(p.area.hr = round(n.area.hr / no.areas, digits = 2),
