...
```
__Add the GCC code into the dataset__

The GCC codes distinguish between 'Greater Sydney' and 'Rest of NSW' and the ACT, allowing for more granular analysis if neccesart.
``` {r}
# Load the list of GCC names
GCC <- read.csv('./achd-dissertation/EDA/output/sa2_GCC_name.csv') %>%
          select(SA2_NAME16, GCC_NAME16) %>%
          rename("sa2_area" = SA2_NAME16)

sa2.data9 <- left_join(sa2.data8, GCC, by = 'sa2_area')
...
sa2.data10 <- left_join(sa2.data9, htt, by = 'SA2_5DIGITCODE_2016') %>%
...
#write.csv(sa2.data10, "./achd-dissertation/EDA/output/sa2_table_builder.csv")
#saveRDS(sa2.data10, file = "./achd-dissertation/EDA/output/sa2_table_builder.rds")
