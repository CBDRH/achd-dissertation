...
ACHD_to_EPCC <- read.csv('./dx_codes/ACHD_EPCC_coding.csv') %>% 
                filter(!(Variable == 'adm_AbsentPA' | Variable == 'PA'))

# Change the DX column names to the correct ECC codes
names(DX) <- ACHD_to_EPCC$EPCC.Code[match(names(DX), ACHD_to_EPCC$Variable)]
...
EPCC_codes <- as.character(ACHD_to_EPCC[["EPCC.Code"]])
dx_names <- as.character(ACHD_to_EPCC[["Adjust.name"]])
...
asgs_lookup <- read.csv("./achd-dissertation/analysis_pipeline/output/ASGS_lookup.csv") %>%
#             ------------Clean up the data-----------------  
...
# Convert to data frame and clean
dx.count.filtered <- as.data.frame(dxCR3.count.filtered)
