...
ACHD_to_EPCC <- read.csv(paste(study_folder, '/dx_codes/ACHD_EPCC_coding.csv', sep = "")) %>% 
                filter(!(ï..Variable == 'adm_AbsentPA' | ï..Variable == 'PA'))

# Change the DX column names to the correct ECC codes
names(DX) <- ACHD_to_EPCC$EPCC_Code[match(names(DX), ACHD_to_EPCC$ï..Variable)]

# Remove the 'achd_id' row from the ACHD to EPCC lookup table
ACHD_to_EPCC <- filter(ACHD_to_EPCC, !(ï..Variable == "achd_id"))
```
...
write.xlsx(x = master_code_sheet,
           file = paste(ACHD_path, 'master_code_sheet/RPAH_master_code_sheet_', Sys.Date(), '.xlsx', sep=""),
...
saveRDS(analysis_dataset, file = paste(ACHD_path, 'output/rpah_analysis_dataset_', Sys.Date(), '.rds', sep=""))
#cannot save the nested 'clinics columns as csv
analysis_dataset %>% select(-clinics,-clinics_2000) %>%
      write.csv(paste(ACHD_path, 'output/rpah_analysis_dataset_', Sys.Date(), '.csv', sep=""))
```
