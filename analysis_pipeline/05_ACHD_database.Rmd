...
ACHD_path <- 'Z:/CURRENT_STUDIES/2020_achd_map/DATA/ACHD_TABLES_2022/'
...
person.level <- person.level %>%
            mutate(
              ...
              # time since last clinic attendance
              gap_2000 = case_when(
                death == 0 ~ as.duration(as.Date('31/12/2020', format='%d/%m/%Y') - last_clinic_2000),
...
```
``` {r, echo = FALSE, warning = FALSE}
knitr::kable(person.level %>% count(death))
```
...
person.level.filtered <- person.level %>% # 5244 patients
                            ...
                            filter(valid_address == 1) # 1101 removed, 3488 remain

```

``` {r, echo = FALSE, warning = FALSE}
knitr::kable(person.level.filtered %>% count(death))
```
...
write.csv(x = master_code_sheet,
           file = paste("Z:/CURRENT_STUDIES/2020_achd_map/DATA/master_code_sheet/", "RPAH_master_code_sheet", Sys.Date(), ".csv", sep=""))
...
saveRDS(analysis_dataset, file = paste("Z:/CURRENT_STUDIES/2020_achd_map/DATA/", "output/rpah_analysis_dataset.rds", sep=""))
#cannot save the nested 'clinics columns as csv
analysis_dataset %>% select(-clinics_2000) %>%
      write.csv(paste("Z:/CURRENT_STUDIES/2020_achd_map/DATA/", 'output/rpah_analysis_dataset.csv', sep=""))
```
