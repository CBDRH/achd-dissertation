...
#       ------------Clean up the data-------------------------  
        ...
        select(person_id, SHC, DC, adm_RC, adm_Bethesda, adm_Bethesda_Code, FupStatus, Death_Cause, achd.1) %>%
         rename('achd_id' = person_id,
               'dx_comment' = achd.1,
               'RC' = adm_RC,
...
``` {r}
# Load the MAIN table from the ACHD database
DX <- read.csv(paste(ACHD_path, 'Table_exports/main_Table.csv', sep=""))  %>%
#       ------------Clean up the data-------------------------  
...
write.csv(address_cleaning_list, paste(ACHD_path, 'output/test_', Sys.Date(), '.csv', sep=""))
```
...
write.xlsx(x = master_code_sheet,
           file = paste(ACHD_path, 'master_code_sheet/RPAH_master_code_sheet.xlsx', sep=""),
